...
async def test_allows_up_to_quota_then_raises():
    limiter = RateLimiter(InMemoryQuotaBackend(), daily_quota=2, min_delay=0, max_delay=0)
...
async def test_remaining_today_tracks_usage():
    limiter = RateLimiter(InMemoryQuotaBackend(), daily_quota=5, min_delay=0, max_delay=0)
...
async def test_accounts_have_independent_buckets():
    ...
    limiter = RateLimiter(InMemoryQuotaBackend(), daily_quota=1, min_delay=0, max_delay=0)
